...
function payloadRecord(task: Task, key: string): Record<string, unknown> {
  const value = task.input_payload[key];
...
export function readinessBadgesForTask(task: Task, asset?: Asset): ReadinessBadge[] {
  ...
  const pairGenerationMetadata = payloadRecord(task, "pair_generation_metadata");
  const pairGenerationStrategy = typeof pairGenerationMetadata.strategy === "string" ? pairGenerationMetadata.strategy : payloadString(task, "pair_generation_strategy");
  ...
  if (task.input_payload.live_model_call_used || pairGenerationMetadata.live_model_call === true) {
    badges.push({ label: "Live AI", tone: "good", tooltip: "This draft includes output from a live model call and still requires Adam review before downstream use." });
  }
  if (pairGenerationMetadata.no_live_model_call === true) {
    badges.push({ label: "No model call", tone: "neutral", tooltip: "This prompt-pair candidate came from deterministic extraction/fallback, not a live model call." });
  }
  if (pairGenerationStrategy.includes("fallback") || pairGenerationStrategy.includes("scaffold")) {
    badges.push({ label: "Fallback", tone: "warning", tooltip: "This item was generated by a deterministic fallback/scaffold path and should be treated as a draft only." });
  }
  if (pairGenerationMetadata.source_text_sha256) {
    badges.push({ label: "Evidence-linked", tone: "info", tooltip: "This item carries source text provenance for review/export manifests." });
  }
