...
from app.exports.jsonl import dpo_export_items, sft_export_items, to_jsonl
...
def build_dataset_export(
    ...
    if payload.export_type == "sft":
        items = sft_export_items(session)
    elif payload.export_type == "dpo":
        items = dpo_export_items(session)
    else:
        items = []
    ...
    export = DatasetExport(
        ...
        status="built",
        manifest={"item_count": len(items), "format": "jsonl"},
    )
    session.add(export)
    session.flush()
